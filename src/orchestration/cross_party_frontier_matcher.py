...
from src.orchestration.private_semantic_routing import SemanticRelationRouting, PrivateSemanticRelationRouter
...
from src.semantic.relation_buckets import RelationSemanticIndex
...
class CrossPartyFrontierMatcher:
    ...
    def match_multi_hop(self, query_graph: Sequence[QueryEdge]) -> list[OpaquePathShare]:
        ...
        states = self._initial_states(edges[0], exact, semantic_routing)
        if not states:
            return []
        if len(edges) == 1:
                ...
                for target_id in party.adjacency.get(source_id, {}).get(relation_id, []):
                    ...
                    states.append(
                        FrontierPathState(
                            ...
                            score=self._relation_score(relation_label, relation_id, exact),
                        )
                    )
        return states
                ...
                for source_id in frontier_indexes[party.party_id].entities_for(frontier_match.frontier_token):
                    for relation_id in relation_ids:
                        for target_id in party.adjacency.get(source_id, {}).get(relation_id, []):
                            ...
                            path_score = previous.score + self._relation_score(edge[1], relation_id, exact)
                            if is_final:
    ...
    def _semantic_relation_routing(
        ...
        semantic_indexes = {
            party.party_id: RelationSemanticIndex.from_secure_index(party, self.ids)
            for party in self.parties
        }
        return PrivateSemanticRelationRouter(self.ids, self.backend, party_ids).route(labels, semantic_indexes)
    ...
    def _relation_score(self, relation_label: str, relation_id: str, exact: ExactQueryIds) -> float:
        return 0.0 if relation_id == exact.relation_ids[relation_label] else self.semantic_relation_penalty
