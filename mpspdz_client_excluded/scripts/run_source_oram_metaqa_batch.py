#!/usr/bin/env python3
"""Run a MetaQA query batch with source-only MP-SPDZ ORAM initialization.

This optimized path uses the offline reverse logical rows already present in the
source adjacency index. For a second hop `middle --r--> target`, it privately
looks up `target --r(reverse)--> middle` in the same source ORAM and swaps the
row back to the logical output orientation after controlled reveal.
"""
...
from prepare_metaqa_twohop_mpspdz_inputs import _hmac_int, _is_unknown, _load_manifest, _repo_root, _write_input_file
...
def _normalize_query(row: dict, setup_key: str, semantic_relations: bool) -> tuple[dict, list[int]]:
    ...
    if len(original_edges) == 1:
        ...
        raise ValueError(f"source-only ORAM batch supports one or two edges, got {len(original_edges)}")

    first, second = original_edges
    relation_1, direction_1 = _normalize_relation(first[1], semantic_relations=semantic_relations)
    type_mode = bool(semantic_relations and _is_type_relation(second[1]))
    if type_mode:
        relation_2, direction_2 = TYPE_IDENTITY_RELATION, "forward"
    else:
        relation_2, direction_2 = _normalize_relation(second[1], semantic_relations=semantic_relations)
    ...
    source_id = _hmac_int(setup_key, "entity", first[0])
    relation_1_id = _hmac_int(setup_key, "relation", relation_1)
    relation_2_id = _hmac_int(setup_key, "relation", relation_2)
    target_id = 0 if target_is_unknown else _hmac_int(setup_key, "entity", second[2])
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="mpc",
        help=(
            "mpc keeps MP-SPDZ private top-k as the ranking backend. "
            "local-gc-prototype reranks only MP-SPDZ-selected paths with LocalGarbledCircuitTopK."
        ),
    )
    ...
    base_query_values = []
    for row in rows:
        normalized, values = _normalize_query(row, setup_key, args.semantic_relations)
    ...
    template = (
        ...
        / "programs"
        / "secure_kg_twohop_source_oram_batch_topk.mpc.template"
    ).read_text()
    ...
    program_name = "secure_kg_twohop_source_oram_batch_topk"
    ...
    receipt = {
        "format": "fedkg-mpspdz-source-oram-batch-v1",
        "index_dir": str(index_root.resolve()),
        "data_parties": [{"party_id": party_id} for party_id in party_ids],
        "queries": normalized_queries,
        "optimization": (
            "Uses only source_directory/source_edges. The second hop is looked up via offline reverse "
            "logical rows and decoded back to the requested path orientation after top-k reveal."
        ),
    }
    ...
    print(f"Prepared {len(rows)} private queries with source-only ORAM state.", flush=True)
    started = time.perf_counter()
    run_env = os.environ.copy()
    run_env["MP_SPDZ_HOME"] = args.mp_spdz_home
    run_env["FEDKG_MPSPDZ_PROGRAM"] = program_name
    mpc = _run(
    ...
    for query_number, (query, handles) in enumerate(zip(normalized_queries, selected)):
        ...
        record = {
            **query,
            "status": "ok",
            "source_oram_mpc_correct": is_correct,
    ...
    summary = {
        ...
        "optimized_path": "source-only-oram",
        "ranking_backend": args.ranking_backend,
    }
    if args.ranking_backend == "local-gc-prototype":
        summary["ranking_security_note"] = (
            "LocalGarbledCircuitTopK is integrated as a prototype reranker over already selected "
            "MP-SPDZ outputs. Hidden-candidate top-k remains inside MP-SPDZ."
        )
