...
func main() {
	parties := flag.Int("parties", 3, "number of threshold PIR parties")
	...
	t := *threshold
	if N < 3 || N > 128 {
		l.Fatal("parties must be in the range [3, 128]")
	}
	if t < 2 || t >= N {
		l.Fatal("threshold must be in the range [2, parties-1]")
	}
	if *nGoRoutine < 1 {
		l.Fatal("go-routines must be at least 1")
	}
	if *recordsPath == "" {
	...
	indices, err := parseIndices(*queryIndices, *queryIndex, len(records))
	check(err)
	if len(records) > params.N() {
		l.Fatalf("database has %d records but current PIR vector supports at most %d", len(records), params.N())
	}
	...
	encInputs := make([]*rlwe.Ciphertext, len(recordVectors))
	...
	elapsedEncryptParty := runTimed(func() {
		for i, row := range recordVectors {
			if err := encoder.Encode(row, pt); err != nil {
				panic(err)
			}
	...
	runQueries := func(indices []int) []map[string]any {
		...
		for _, idx := range indices {
			resetRequestTimers()
			encQuery := genQuery(params, idx, encoder, encryptor)
			encResult := execRequest(params, *nGoRoutine, encQuery, encInputs, plainMask, evk)
	...
	staticTiming := map[string]string{
		"setup_cloud":      (elapsedCKGCloud + elapsedRKGCloud + elapsedGKGCloud).String(),
		"setup_party":      (elapsedCKGParty + elapsedRKGParty + elapsedGKGParty).String(),
		"input_encryption": elapsedEncryptParty.String(),
		"setup_plus_input": (elapsedCKGCloud + elapsedRKGCloud + elapsedGKGCloud + elapsedEncryptParty).String(),
	}
	...
	if *serverStdin {
		...
		for scanner.Scan() {
			...
			if len(request.Indices) == 0 {
				encoded, _ := json.Marshal(map[string]any{"error": "indices is required"})
				fmt.Println(string(encoded))
				continue
			}
			valid := true
			...
			if !valid {
				encoded, _ := json.Marshal(map[string]any{"error": "index out of range"})
				fmt.Println(string(encoded))
				continue
			}
	...
	l.Printf("> Finished (total cloud: %s, total party: %s)\n",
		...
		elapsedCKGParty+elapsedRKGParty+elapsedGKGParty+elapsedEncryptParty)
...
func parseIndices(raw string, fallback int, size int) ([]int, error) {
	if raw == "" {
		if fallback < 0 || fallback >= size {
	...
	if len(indices) == 0 {
		return nil, errors.New("indices is empty")
	}
	return indices, nil
}

func resetRequestTimers() {
	elapsedCKSCloud = 0
	elapsedCKSParty = 0
	elapsedRequestParty = 0
	elapsedRequestCloud = 0
...
func loadRecordVectors(path string, recordSize int, slots int) ([]string, [][]uint64, error) {
	...
	if recordSize > slots {
		return nil, nil, fmt.Errorf("record-size %d exceeds available slots %d", recordSize, slots)
	}
...
func decodeRecord(values []uint64, recordSize int) []byte {
	out := make([]byte, 0, recordSize)
	for i := 0; i < recordSize && i < len(values); i++ {
		value := values[i]
		if value == 0 {
			break
		}
...
func precomputePlainMasks(params bgv.Parameters, inputCount int, encoder *bgv.Encoder) []*rlwe.Plaintext {
	plainMask := make([]*rlwe.Plaintext, inputCount)
	...
	l.Printf("\tdone (masks: %d)\n", len(plainMask))
	return plainMask
}

func execRequest(params bgv.Parameters, NGoRoutine int, encQuery *rlwe.Ciphertext, encInputs []*rlwe.Ciphertext, plainMask []*rlwe.Plaintext, evk rlwe.EvaluationKeySet) *rlwe.Ciphertext {
