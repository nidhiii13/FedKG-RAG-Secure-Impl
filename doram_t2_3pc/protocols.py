...
class Protocol:
    ...
    malicious: bool
    summary: str
    prime_congruence_modulus: int = 1
    ...
    def validate_field_prime(self, prime: int) -> None:
        """Fail before launch when the protocol cannot represent the field.

        Hemi and Temi use NTT-based homomorphic encryption.  The 128-bit
        parameter set generated by MP-SPDZ uses a cyclotomic degree of 32768,
        so accepting only primes congruent to one modulo that degree is a
        conservative, deterministic preflight check.  OT-based protocols have
        no additional congruence requirement here.
        """

        modulus = self.prime_congruence_modulus
        if prime % modulus != 1 % modulus:
            raise ValueError(
                f"protocol {self.key!r} requires field_prime == 1 mod "
                f"{modulus}; got {prime}. Generate a new configuration and "
                "new input shares under the audited HE-compatible field; "
                "existing shares cannot be reused across fields."
            )
...
PROTOCOLS: dict[str, Protocol] = {
    ...
    "hemi": Protocol(
        ...
        malicious=False,
        summary=(
            "Matches the declared threat model. Semi-honest, dishonest "
            "majority, mod prime. MP-SPDZ generates additive Beaver triples "
            "with semi-homomorphic encryption and uses diagonal packing for "
            "matrix multiplication. Results must be reported separately from "
            "the OT-based Semi baseline."
        ),
        prime_congruence_modulus=32768,
    ),
    "temi": Protocol(
        key="temi",
        script="temi.sh",
        ...
        summary=(
            "Matches the declared threat model. Semi-honest, dishonest "
            "majority, mod prime. MP-SPDZ uses its LWE-based adaptation of "
            "CDN01 and diagonal packing for matrix multiplication. Results "
            "must be reported separately from both Semi and Hemi."
        ),
        prime_congruence_modulus=32768,
    ),
    "mascot": Protocol(
